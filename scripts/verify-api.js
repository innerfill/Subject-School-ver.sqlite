...
async function verify() {
    ...
    const conflict1 = await fetch(`${BASE_URL}/schedules`, {
        ...
        body: JSON.stringify({
            teacher_id: teacher.id,
            subject_id: subject.id, // Same subject or different doesn't matter
            room_id: room.id + 1, // Different room (assuming it might exist or not, but logic checks teacher)
            class_id: cls.id + 1, // Different class
            day_of_week: 'Monday',
            start_time: '09:00:00',
            end_time: '10:00:00'
        })
